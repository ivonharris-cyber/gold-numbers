...
export function luckyDigitsForSign(sign) {
    const idx = STAR_SIGNS.indexOf(sign);
    if (idx === -1)
        throw new Error(`unknown star sign: ${sign}`);
 ...
 */
function buildWeights(stats, lucky) {
...
function pickDigit(weights, rng) {
    let total = 0;
...
function makeNumber(width, weights, rng) {
    let out = '';
    for (let i = 0; i < width; i++)
        out += String(pickDigit(weights, rng));
 ...
 */
export function generateGoldSets(data, starSign, dateSalt) {
    const stats = analyse(data);
    const lucky = luckyDigitsForSign(starSign);
    const totalDigits = stats.counts.reduce((a, b) => a + b, 0);
    const seed = (fnv1a(`${starSign}:${dateSalt ?? 'gold'}`) ^
        fnv1a(`${stats.hot.join('')}:${totalDigits}`)) >>> 0;
    ...
    while (sets.length < NUM_SETS && guard < NUM_SETS * 100) {
        ...
        if (seenSix.has(sixDigit))
            continue; // 6-digit picks must be unique
