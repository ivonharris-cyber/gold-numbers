 ...
 */
import { analyzeFrequency, normalizeDraws, validateDraw } from './frequency.js';
import { isZodiacSign, luckyDigitsFor, signForDate } from './zodiac.js';
...
const W_ZODIAC = 0.25;
function blendPool(pos, favoured, zodiac) {
    const weights = new Array(10).fill(0);
    const max = Math.max(1, ...pos.counts);
    for (let d = 0; d < 10; d++)
        weights[d] += W_HOT * (pos.counts[d] / max);
    for (const d of favoured)
        weights[d] += W_FAVOURED;
    for (const d of zodiac)
        weights[d] += W_ZODIAC;
    return { weights };
}
function pickDigit(pool, rng) {
...
function scoreCandidate(value, pools) {
    let sum = 0;
...
function makeCandidate(width, pools, rng) {
    let out = '';
    for (let i = 0; i < width; i++)
        out += String(pickDigit(pools[i], rng));
    return out;
}
 ...
 */
export function generateSets(sign, favoured, draws, birth) {
    if (!isZodiacSign(sign)) {
        throw new Error(`unknown zodiac sign "${sign}" — expected one of aries..pisces`);
    }
    const flat = normalizeDraws(draws);
    if (flat.length === 0)
        throw new Error('generateSets requires at least one draw');
    for (const d of flat)
        validateDraw(d);
    const birthInfo = normalizeBirth(birth);
    const favouredClean = [...new Set((favoured ?? []).filter((d) => Number.isInteger(d) && d >= 0 && d <= 9))].sort((a, b) => a - b);
    const zodiacDigits = new Set(luckyDigitsFor(sign));
    if (birthInfo) {
        const [, m, d] = birthInfo.date.split('-').map(Number);
        for (const digit of luckyDigitsFor(signForDate(m, d)))
            zodiacDigits.add(digit);
    }
    const zodiac = [...zodiacDigits].sort((a, b) => a - b);
    const freq = analyzeFrequency(flat);
    const seed = hashString(`${sign}|${favouredClean.join(',')}|${birthInfo ? `${birthInfo.date}T${birthInfo.time}` : ''}`);
    ...
    while (sets.length < NUM_SETS && guard < NUM_SETS * 50) {
        ...
        if (seen.has(key))
            continue;
        seen.add(key);
        const score = scoreCandidate(sixDigit, sixPools) * 0.5 +
 ...
 */
export function hotColdDigits(draws) {
