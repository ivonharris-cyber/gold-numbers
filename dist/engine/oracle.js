...
export function ticketOracle(birth) {
    ...
    // Auspicious 2-hour buy window anchored on the birth hour, nudged by the hash
    const start = (hh + (h % 3)) % 22; // 00..21 so the window stays inside the day
    const pad = (n) => String(n).padStart(2, '0');
    const time = `${pad(start)}:00-${pad(start + 2)}:00`;
