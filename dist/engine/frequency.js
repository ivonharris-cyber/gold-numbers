...
function buildPositionStats(width, values) {
    const positions = [];
...
export function validateDraw(d) {
    if (!/^\d{6}$/.test(d.firstPrize))
        throw new Error(`draw ${d.date}: firstPrize must be 6 digits`);
    for (const v of d.front3)
        if (!/^\d{3}$/.test(v))
            throw new Error(`draw ${d.date}: front3 entries must be 3 digits`);
    for (const v of d.last3)
        if (!/^\d{3}$/.test(v))
            throw new Error(`draw ${d.date}: last3 entries must be 3 digits`);
    if (!/^\d{2}$/.test(d.last2))
        throw new Error(`draw ${d.date}: last2 must be 2 digits`);
}
/** Accept both the flat Draw[] shape and the wrapped DrawsFile shape
 *  ({ draws: [...] }) so callers can pass fetched JSON directly. */
export function normalizeDraws(input) {
    const arr = Array.isArray(input) ? input : input.draws;
    if (!Array.isArray(arr))
        throw new Error('draw data must be an array or { draws: [...] }');
...
export function analyzeFrequency(draws) {
    ...
    for (const d of draws) {
        ...
        for (const c of all)
            overallCounts[c.charCodeAt(0) - 48]++;
    }
    return {
        firstPrize,
        threeDigit,
