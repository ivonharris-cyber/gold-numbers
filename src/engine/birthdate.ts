...
export interface BirthDigits {
  birthDayDigit: number;
  lifePath: number;
  timeDigit: number;
  weekDayDigit: number;
  /** de-duplicated, sorted ascending — ready to feed a digit pool */
  all: number[];
}
 ...
 */
export function birthDigits(date: string, time: string): BirthDigits {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new Error(`birth date must be yyyy-mm-dd, got "${date}"`);
  if (!/^\d{2}:\d{2}$/.test(time)) throw new Error(`birth time must be HH:MM, got "${time}"`);

  const [y, m, d] = date.split('-').map(Number);
  const [hh, mm] = time.split(':').map(Number);
  if (m < 1 || m > 12 || d < 1 || d > 31) throw new Error(`birth date out of range: "${date}"`);
  if (hh > 23 || mm > 59) throw new Error(`birth time out of range: "${time}"`);

  const birthDayDigit = digitalRoot(d);
  const lifePath = digitalRoot(
    String(y).split('').reduce((a, c) => a + Number(c), 0)
